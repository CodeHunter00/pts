...
class Window(Frame):
    def __init__(self, master=None):
        ...
        B1 = Button(F1, text="Proje Ekle", command=self.ekle, font="bold", cursor="hand2", fg="red", width=13, height=3)
        B1.grid(row=0, column=0, padx=20)
        B2 = Button(F1, text="Proje Listele", command=self.listele, font="bold", cursor="hand2", fg="green", width=13,
                    height=3)
    ...
    def ekle(self):
        ...
        Label(self.F_ekle, text="Proje Adı: ").grid(row=0, column=0, pady=5, sticky=W)
        self.E1 = Entry(self.F_ekle, width=35)
        self.E1.grid(row=0, column=1, pady=5)

        Label(self.F_ekle, text="Yönetici: ").grid(row=1, column=0, pady=5, sticky=W)
        self.E2 = Entry(self.F_ekle, width=35)
        self.E2.grid(row=1, column=1, pady=5)

        Label(self.F_ekle, text="Bölüm: ").grid(row=2, column=0, pady=5, sticky=W)
        self.E3 = Entry(self.F_ekle, width=35)
        self.E3.grid(row=2, column=1, pady=5)

        Label(self.F_ekle, text="Kişi Sayısı: ").grid(row=3, column=0, pady=5, sticky=W)
        self.SS = Spinbox(self.F_ekle, from_=1, to=999, width=5)
        self.SS.grid(row=3, column=1, pady=5, sticky=W)

        Label(self.F_ekle, text="Açıklama: ").grid(row=4, column=0, pady=5, sticky=W)
        self.SSS = Entry(self.F_ekle, width=35)
        self.SSS.grid(row=4, column=1, pady=5, sticky=W)

        Label(self.F_ekle, text="Başlangıç Tarihi (GG.AA.YYYY): ").grid(row=5, column=0, pady=5, sticky=W)
    ...
    def kayit_et(self):
        try:
            self.im.execute(
                "CREATE TABLE IF NOT EXISTS projeler (id INTEGER PRIMARY KEY, Proje_Adi VARCHAR(45), Yonetici VARCHAR(45), Bolum VARCHAR(45), Kisi_Sayisi INT, Aciklama VARCHAR(45), Baslangic_Tarihi)")  # Tablo oluşturma
            self.im.execute(
                "INSERT INTO projeler VALUES (null,'" + self.E1.get() + "','" + self.E2.get() + "','" + self.E3.get() + "','" + self.SS.get() + "','" + self.SSS.get() + "','" + self.BasimYil.get() + "')")  # Veri ekleme
    ...
    def listele(self):
        try:
            ...
            self.im.execute("SELECT * FROM projeler")
            data = self.im.fetchall()

            def search():
                deget = self.ara.get()
                query = "SELECT id, Proje_Adi, Yonetici, Bolum, Kisi_Sayisi, Aciklama, Baslangic_Tarihi FROM projeler WHERE Proje_Adi LIKE '%" + deget + "%' OR Yonetici LIKE '%" + deget + "%' OR Bolum LIKE '%" + deget + "%'"
            ...
            self.tv.heading(2, text='Proje Adı')
            self.tv.heading(3, text='Yönetici')
            self.tv.heading(4, text='Bölüm')
            self.tv.heading(5, text='Kişi Sayısı')
            self.tv.heading(6, text='Açıklama')
            self.tv.heading(7, text='Başlangıç Tarihi')
    ...
    def Duzenle(self):
        ...
        def veriKayit():
            ...
            self.im.execute(
                "CREATE TABLE IF NOT EXISTS projeler (Proje Adı VARCHAR(45), Yönetici VARCHAR(45), Bölüm VARCHAR(45), Kişi Sayısı INT, Açıklama VARCHAR(45), Başlangıç Tarihi)")  # Tablo oluşturma

            self.im.execute(
                "UPDATE projeler SET Proje_Adi = ?, Yonetici = ?, Bolum = ?, Kisi_Sayisi = ?, Aciklama = ?, Baslangic_Tarihi = ? WHERE id = ?",
                (self.E1.get(), self.E2.get(), self.E3.get(), self.SS.get(), self.SSS.get(), self.BasimYil.get(),
                 numara))
            self.baglanti.commit()
        ...
        def veriSil():
            ...
            if evet:
                self.im.execute("DELETE FROM projeler WHERE id = ?", [numara])
                self.baglanti.commit()
                yeniWin.destroy()
        ...
        F_ekle.pack()
        kopya = self.tv.focus()
        Label(F_ekle, text="Proje: ").grid(row=0, column=0, pady=5, sticky=W)
        ...
        self.E1.grid(row=0, column=1, pady=5)
        Label(F_ekle, text="Yönetici: ").grid(row=1, column=0, pady=5, sticky=W)
        self.E2 = Entry(F_ekle, width=35)
        self.E2.insert(0, self.tv.item(kopya)["values"][2])
        self.E2.config(state="disable")
        self.E2.grid(row=1, column=1, pady=5)
        Label(F_ekle, text="Bölüm: ").grid(row=2, column=0, pady=5, sticky=W)
        ...
        Label(F_ekle, text="Kişi Sayısı: ").grid(row=3, column=0, pady=5, sticky=W)
        self.SS = Spinbox(F_ekle, width=5)
        self.SS.insert(0, self.tv.item(kopya)["values"][4])
        self.SS.config(state="disable")
        self.SS.grid(row=3, column=1, pady=5, sticky=W)

        Label(F_ekle, text="Açıklama: ").grid(row=4, column=0, pady=5, sticky=W)
        self.SSS = Entry(F_ekle, width=35)
        self.SSS.insert(0, self.tv.item(kopya)["values"][5])
        self.SSS.config(state="disable")
        self.SSS.grid(row=4, column=1, pady=5, sticky=W)

        Label(F_ekle, text="B. Tarihi (GG.AA.YYYY): ").grid(row=5, column=0, pady=5, sticky=W)
...
root = Tk()
app = Window(root)
root.wm_title("Proje Takip Sistemi")
...
me = Label(root, text="InnovateTech | CodeHunter00", fg="#6E7371", cursor="hand2",
           font="Verdana 7 bold")
me.pack(side=BOTTOM)
me.bind("<Button-1>", lambda e: callback(webbrowser.open_new("https://www.instagram.com/kod.arsivim/")))
